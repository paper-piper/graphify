...
import { EDGE_STATUS, CreateEdgeStatus } from '../status/statuses';
...
export async function CreateEdgeService(source_node_title: NodeTitle, target_node_title: NodeTitle): Promise<CreateEdgeStatus> {
    ...
    if (source_node_id === null || target_node_id === null) return EDGE_STATUS.NODE_NOT_FOUND;
    const success = await create_edge(source_node_id, target_node_id);
    return success ? EDGE_STATUS.CREATED : EDGE_STATUS.ALREADY_EXISTS;
}
